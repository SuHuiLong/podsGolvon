...
- (void)createToolbar
{
    ...
    [_toolbar setBlock:^(BOOL hasSave) {
        [weakSelf showAlertTitileViewWithTitle:@"保存成功"];
    }];
...
-(void)showAlertTitileViewWithTitle:(NSString*)title
{
    UILabel*label=[[UILabel alloc]init];
    
    UIView*coverView=[[UIView alloc]init];
    coverView.center=CGPointMake(self.view.centerX, self.view.centerY);
    coverView.bounds=CGRectMake(0, 0, 194, 76);
    coverView.backgroundColor=[UIColor colorWithRed:0 green:0 blue:0 alpha:0.2f];
    coverView.tag=200;
    coverView.layer.cornerRadius=5;
    [self.view addSubview:coverView];
    
    label.frame=CGRectMake(0, 28, 194, 20);
    label.text=title;
    label.textColor=[UIColor whiteColor];
    label.textAlignment=NSTextAlignmentCenter;
    [coverView addSubview:label];
    
    [self performSelector:@selector(hidenTheBackView) withObject:nil afterDelay:1.2];
}

-(void)hidenTheBackView
{
    UIView*coverView=[self.view viewWithTag:200];

...
#define indicatorViewColorDefualt localColor
...
- (void)selectedBtnLocation:(UIButton *)button {
    ...
    if (self.segmentedControlIndicatorType == segmentedControlIndicatorTypeCenter) {
        ...
        [UIView animateWithDuration:indicatorViewTimeOfAnimation animations:^{
            self.indicatorViewWithImage.centerX = button.centerX;
        }];
...
- (void)setSegmentedControlIndicatorType:(segmentedControlIndicatorType)segmentedControlIndicatorType {
    ...
    if (self.segmentedControlIndicatorType == segmentedControlIndicatorTypeBottom) {
        ...
        self.indicatorView.height = indicatorViewHeight_c;
        self.indicatorView.top = (self.frame.size.height - indicatorViewHeight_c) * 0.5;
        ...
        // 改变原指示器样式
        _indicatorView.left = firstButton.left;
        _indicatorView.width = buttonSize.width + 2 * btn_Margin;
        CGFloat indicatorViewHeight_c = self.frame.size.height;
        self.indicatorView.height = indicatorViewHeight_c;
        self.indicatorView.top = 0;
        self.indicatorView.backgroundColor = [textTintColor colorWithAlphaComponent:0.3];
        
        // 增加新的指示器（指示器底部的小指示器）
        self.bgIndicatorView = [[UIView alloc] init];
        _bgIndicatorView.backgroundColor = localColor;
        _bgIndicatorView.left = firstButton.left;
        _bgIndicatorView.width = _indicatorView.width;
        _bgIndicatorView.height = indicatorViewHeight;
        _bgIndicatorView.top = _indicatorView.height - indicatorViewHeight;
        ...
        _indicatorViewWithImage.height = _indicatorViewWithImage.image.size.height;
        _indicatorViewWithImage.top = self.height - _indicatorViewWithImage.height;
        _indicatorViewWithImage.width = _indicatorViewWithImage.image.size.width;
        _indicatorViewWithImage.centerX = firstButton.centerX;

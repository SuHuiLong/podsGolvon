...
@property (nonatomic, copy) JCTagListViewBlock DeselectedBlock;

@property(nonatomic, assign)NSInteger selectNum;
@property(nonatomic, strong)NSMutableArray *selectItemArry;
...
-(NSMutableArray *)selectItemArry{
    if (_selectItemArry == nil) {
        _selectItemArry = [NSMutableArray array];
    }
    return _selectItemArry;
}
...
- (void)setCompletionBlockWithDeSelected:(JCTagListViewBlock)completionBlock
{
    self.DeselectedBlock = completionBlock;
}
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout*)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    CGRect frame = [self.tags[indexPath.item] boundingRectWithSize:maxSize options:NSStringDrawingUsesFontLeading|NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName: [UIFont systemFontOfSize:16.0f]} context:nil];
    
    return CGSizeMake(frame.size.width + WScale(7), layout.itemSize.height);
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self.selectedTags containsObject:self.tags[indexPath.item]]) {
        _selectNum = _selectedTags.count;
//        _selectItemArry = 
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    if (self.canSelectTags) {
        ...
        if ([self.selectedTags containsObject:self.tags[indexPath.item]]) {
            ...
            self.DeselectedBlock(indexPath.item);
        }
        else {
            if (_selectNum>5) {
                return;
            ...
            cell.backgroundColor = self.tagSelectedBackgroundColor;
            cell.titleLabel.textColor = self.tagTextSelectedBackgroundColor;

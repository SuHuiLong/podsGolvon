...
-(void)createBottomBar{
    ...
    UIButton *DoneButton = [Factory createButtonWithFrame:CGRectMake( ScreenWidth - kWvertical(81), kHvertical(9), kWvertical(70), kHvertical(28))titleFont:14.0f textColor:WhiteColor backgroundColor:localColor target:self selector:@selector(doneClick:) Title:SelectNum];
...
- (void)setupPage:(NSInteger )sender
{
    ...
    NSString *imageStr = [NSString new];
    
    if ([_dataArray[sender] isKindOfClass:[PictureModel class]]) {
        PictureModel *model = _dataArray[sender];

        [pImageView setImageWithUrl:model.url];
        pImageView.tag = 1000+sender;

        
    }else if ([_dataArray[sender] isKindOfClass:[NSData class]]) {
    ...
    if ([_logInController isEqualToString:@"PublishPhotoViewController"]) {
        id fullImage = _dataArray[sender];
        imageStr = [fullImage objectForKey:@"fullScreenImage"];
    
    }else{
        TotalPhotoModel *model = _dataArray[sender];
        imageStr=[model.url objectForKey:@"fullScreenImage"];
    }
...
-(void)sendClick:(UITapGestureRecognizer *)gestureRecognizer{
    if (gestureRecognizer.numberOfTouches == 1) {
        if ([_logInController isEqualToString:@"HomePageViewController"]) {
...
-(void)imageSlect:(UIButton *)btn{
    ...
    if (btn.selected) {
        ...
        NSMutableArray *selectArray = [NSMutableArray arrayWithArray:[NSKeyedUnarchiver unarchiveObjectWithData:data]];
        ...
        if (total>=6) {
            dispatch_async(dispatch_get_main_queue(), ^{
                UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"提示" message:@"最多显示六张照片" preferredStyle:UIAlertControllerStyleAlert];
...
-(void)doneClick:(UIButton *)btn{
    ...
    if ([userDefaults objectForKey:@"PublishPhotosArray"]) {
        totalArray = [NSMutableArray arrayWithArray:[userDefaults objectForKey:@"PublishPhotosArray"]];

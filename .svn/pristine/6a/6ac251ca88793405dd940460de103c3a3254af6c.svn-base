...
-(void)clickComplete{
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.8 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        for (NewStartScoringViewController *VC in self.navigationController.viewControllers) {
            
//            if ([VC isKindOfClass:[NewStartScoringViewController class]]) {
...
-(void)requestWithData{
    ...
    [self.loadData downloadWithUrl:[NSString stringWithFormat:@"%@Golvon/insertCustomQiuChang",apiHeader120] parameters:parameters complicate:^(BOOL success, id data) {
        if (success) {
            
            if ([data[@"code"] isEqualToString:@"1"]) {
                weakself.ballParkName = data[@"qiuchang_name"];
                weakself.ballParkID   = data[@"qiuchang_id"];
                ...
                NSArray *poleArray = @[pole0,pole1,pole2,pole3];
                
                NearParkModel *model = [[NearParkModel alloc] init];
                model.qid = _ballParkID;
                model.qname = _ballParkName;
                model.pardata = poleArray;
                weakself.newPark(model);
...
-(void)createNoneHistory{
    ...
    noneLabel.frame         = CGRectMake(0, kHvertical(122), ScreenWidth, kHvertical(20));
...
-(void)deleteHistoryBallPark:(BallParkSelectModel *)model{
    __weak typeof(self) weakself = self;
    NSDictionary *parameters = @{
                                 @"qiuchang_id":model.historyBallParkID
                                 };
    [self.loadData downloadWithUrl:[NSString stringWithFormat:@"%@Golvon/deleteCustomQiuChang",urlHeader120] parameters:parameters complicate:^(BOOL success, id data) {
        if (success) {
            ...
            [weakself requestHistoryBallPark];

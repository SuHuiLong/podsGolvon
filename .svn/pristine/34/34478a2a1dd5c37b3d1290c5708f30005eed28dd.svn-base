...
@property(nonatomic,strong)UIImageView  *MatteView;//蒙版
...
- (void)createBrowseView
{
    ...
    _MatteView = [Factory createImageViewWithFrame:CGRectMake(0, 0, ScreenWidth, HScale(11.8)) Image:[UIImage imageNamed:@"朋友圈照片蒙版"]];
    _MatteView.hidden = NO;
    [self.view addSubview:_MatteView];
    // 2.保存按钮
    UIButton *saveButton = [[UIButton alloc] init];
    saveButton.frame = CGRectMake(WScale(84.5), HScale(4.5), WScale(15.5), HScale(3.3));
    [saveButton setTitle:@"保存" forState:UIControlStateNormal];
    [saveButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    saveButton.backgroundColor = ClearColor;
...
- (void)saveImage
{
    ...
    MSSBrowseCollectionViewCell *cell = (MSSBrowseCollectionViewCell *)[_collectionView cellForItemAtIndexPath:indexPath];

    [self browseActionSheetDidSelectedAtIndex:index currentCell:cell];
}
...
- (void)tap:(MSSBrowseCollectionViewCell *)browseCell
{
    ...
    [_MatteView removeFromSuperview];
